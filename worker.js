...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // ---------------- LIVE REAL-TIME AUDIT MONITOR ---------------- //
      if (pathname === "/api/admin/live-monitor" && method === "GET") {
      ...
      if (pathname === "/api/admin/live-monitor/nudge" && method === "POST") {
        if (!requireAdmin(request)) return jsonResponse({ ok: false, code: "UNAUTHORIZED" }, 401);
        const { openId, name, type, devices } = await request.json().catch(() => ({}));
        if (!openId) return jsonResponse({ ok: false, message: "openId required" }, 400);
